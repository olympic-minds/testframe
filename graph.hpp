...
public:
    ...
    std::uint64_t getNumberOfNodes() const { return graph.size() ; }

    std::uint64_t getNumberOfEdges() const { 
        return std::accumulate(graph.begin(), graph.end(), 0, [](const std::uint64_t x, const auto &a) { 
            return x + a.size(); 
        }); 
    }
    ...
    operator std::vector<std::vector<std::uint64_t>>() const { return graph; }

    std::vector<std::pair<std::uint64_t, std::uint64_t>> getEdges() const {
        std::vector<std::pair<std::uint64_t, std::uint64_t>> edges;
        for (std::uint64_t v = 0; v < getNumberOfNodes() ; ++v)
            for (std::uint64_t u : graph[v]) edges.emplace_back(v, u);
        return edges;
    }
    ...
    static Graph constructSimplerJellyfishGraph(std::uint64_t nodes, std::uint64_t cycleSize, std::uint64_t maxTentacleLength,
                                                   std::uint64_t numberOfTentacles);
    static Graph constructStarfishGraph(std::uint64_t nodes, std::uint64_t maxRayLength, std::uint64_t numberOfRays);
